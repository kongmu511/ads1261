...
Launch GUI:
```bash
python python/wifi_receiver_gui.py
```

The GUI runs only on Windows (it talks to `netsh`) and lets you:

- scan available Wi-Fi networks from the AP, WPS, or nearby routers
- list the currently connected network and its signal strength
- connect to / disconnect from `ZPlate` so the ESP32 can stream data
- start/stop UDP streaming while observing four live plots
...
├── python/
│   └── wifi_receiver_gui.py   # Real-time visualization + Wi-Fi manager
├── docs/
│   └── ZForce_ESP32C6.pdf     # Hardware schematic
├── serial_csv_capture.py      # Legacy serial-to-CSV helper (was `python`)
└── README.md              # This file
```
...
## Legacy serial capture

The previous helper script has been renamed to `serial_csv_capture.py` and still ships from the repo root.
It opens a high-speed serial port, waits for the CSV header, and copies every ADC sample to `adc_data.csv`.

```bash
pip install pyserial
python serial_csv_capture.py
```
